...
RSpec.describe Client, type: :model do
  ...
  describe "#associate_with_company" do
    ...
    it "associates the client with the inccorrect client" do
      expect { client.associate_with_company('123') }.to raise_error(ActiveRecord::RecordInvalid) do |exception|
    ...
    it 'when associates already exists' do
      client.associate_with_company(company.id)
      expect { client.associate_with_company(company.id) }.to raise_error(ActiveRecord::RecordInvalid) do |exception|
        expect(exception.message).to eq('Validation failed: Este cliente já está associado a esta empresa')
      end
    end
  end
  ...
  describe ".with_company_id" do
    ...
    it "returns records associated with the specified company_id" do
      ...
      create(:company_client, client: record, company: company)
