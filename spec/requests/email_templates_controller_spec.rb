...
RSpec.describe 'EmailTemplateController', type: :request do
  ...
  describe 'GET #index' do
    context 'when user is logged in' do
      ...
      context 'when user is a tenant' do
        ...
        before do
          get "/email_templates", headers: { 'Authorization': "Bearer #{token}" }
        end
  ...
  describe 'GET #show' do
    context 'when user is logged in' do
      context 'when email template is found' do
        before do
          ...
          get "/email_templates/#{email_template.id}", headers: { 'Authorization': "Bearer #{token}" }
        end
      ...
      context 'when email template is not found' do
        before do
          get "/email_templates/any_invalid_id"
        end
    ...
    context 'when user is not logger in' do
      before do
        get "/email_templates/#{email_template.id}"
      end
  ...
  describe 'PUT #update' do
    context 'when user is logged in' do
      context 'when user is an admin' do
        ...
        before do
          ...
          put "/email_templates/#{email_template.id}", params: { email_template: email_template_params}, headers: { 'Authorization': "Bearer #{token}" }
        end
      ...
      context 'when user is a employee' do
        ...
        it 'returns a forbidden response' do
          put "/email_templates/#{email_template.id}", headers: { 'Authorization': "Bearer #{token}" }
    ...
    context 'when user is not logger in' do
      before do
        put "/email_templates/#{email_template.id}"
      end
